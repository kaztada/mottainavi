...
| 区分 kind=bulky(または `accepts_large_items` の区分)かつ 名前に「いす/棚/机/タンス/ベッド/ソファ」等(アイス・スライス・コンロ類は除外) | furniture |
...
## 19. 河内長野市(Phase C-3)で確定した事項(2026-09-23)

- **出典**: 市の「家庭用ごみの分別辞典(50音順)」PDF(R8年作成、CC BY 4.0、1,081品目)。出典表記は市の指定どおり「このデータベースは、以下の著作物を改変して利用しています。家庭用ごみの分別辞典、河内長野市」
- **PDF の取り出し**: Node の pdfjs-dist では表の行が崩れるため、pypdf で取り出したテキストを `data/cache/kawachinagano-city.txt` に置き、アダプタはそれを切り出す(`scripts/adapters/kawachinagano-city/extract_text.py`。PDF 更新時のみ手動実行)。`source_type` に `opendata-pdf` を追加
- **切り出し**: 表紙・ページ見出しを捨て、折り返した区分名をつなぎ、「前後に空白のある区分名」で品目行を判定。区分のない行は、直前の備考が文の途中なら続き、そうでなく次の行が品目行なら次の品名の前半。PDF のページ画像と32品目を照合して一致
- **「もえないごみ・粗大ごみ」は kind non-burnable**(bulky にしない)。月1回の通常収集で申込み不要、シール制(毎年無料配布・不足分は有料)。bulky にすると「事前申込みが必要です」と誤案内になる。市のごみガイドブックの説明を注意文言の先頭に置く
- 「通常の収集(集積所からの収集)では回収できません」は家電リサイクル法の品目だけでなくタブレット・パソコンも含むので、区分の説明は付けず市の備考に任せる
- 手放し導線の家具の推定は kind bulky を条件にしていたため、河内長野市のタンス・机などに付かなかった(付与率 4.3%)。**2026-09-25 対応**: categories.json の区分に任意項目 `accepts_large_items: true`(申込制ではないが大型品も出せる)を設け、家具の推定だけ bulky と同じに扱う。kind は non-burnable のまま(申込セクションは出ない)。河内長野市は35品目に家具の導線が付き、付与率 7.5%。あわせて「イス」が「アイスピック」「スライスカッター」に当たる誤判定を除外した(枚方市3品目・横浜市2品目が furniture → なし)

## 20. 平塚市(Phase C・神奈川県)で確定した事項(2026-09-25)

- **出典**: 平塚市オープンデータライブラリ「ごみの分別方法一覧」CSV(805品目、最終更新 2026-02-01)。ライセンスは平塚市オープンデータ利用規約で CC BY 4.0。出典表記は規約の改変時の書式「このデータベースは以下の著作物を改変して利用しています。ごみの分別方法一覧、平塚市、クリエイティブ・コモンズ・ライセンス 表示 4.0 国際」
- **形式**: 国の標準形式(ごみの分別方法)で、東大阪市の CSV と同じ列構成(列名の接頭辞が「ごみの分別方法_」)。原本は CP932。横浜市と同じく UTF-8 に復号したものを `data/cache/hiratsuka-city.csv` に置く。市の品目ID(ff0000000001 等)は連番なので使わない
- **半角カナ**: 品目名・注意文言の半角カナは、連続部分だけ NFKC で全角にする(全体を NFKC にすると全角のかっこ・英数まで変わるため)。区分ラベルの表記ゆれ(「金属（資源再生物）」と「金属(資源再生物)」、「ﾘｻｲｸﾙ」と「リサイクル」)は NFKC で吸収
- **区分**: 21種の表記 → 18区分。処理困難物・家電リサイクル・産業廃棄物は not-collected で、注意文言の先頭に「市では収集しません。」。充電式電池・パソコン・インクカートリッジ(CSV の「リサイクル」)は maker-recycle。剪定枝は無料の各戸収集(電話申込)なので other(bulky にすると粗大ごみの申込セクションが出てしまう)。「不燃ごみ又は処理困難物」(珪藻土バスマット)は2枚のカードに分け、組み合わせ表記の原文を1枚目に残す
- **粗大ごみ**: 事前予約制の各戸収集で有料(原則1立方メートルにつき3,500円、納付券)。申込制なので kind bulky。電話 0463-71-5100 のみでネット申込はないため `sodai_apply_url` は null(「ネットで申込む」を出さない)。市のページの説明を注意文言の先頭に置く。品目ごとの手数料は CSV に無い
- **鮮度の確認**: 市公式のごみサク(gomisaku.jp/0397、328品目)と全件照合し、照合できた238品目中235品目で一致。違う3品目(アルミ箔・ギター・収納ケース)は市の別資料「分別帳一覧」PDF(2024-09)が CSV と一致したので、市の資料を正とした。ごみサクの粗大ごみの電話番号(0463-55-6615)は古く、市のページ(2026-02 更新)の 0463-71-5100 を使う
- **手放し導線**: 付与率 16.1%。「モーター(水槽用・おもちゃ用)」「ビデオテープ保管用カバー」の誤判定3品目は reuse-overrides で null にした(共通の判定を変えると他市の出力が変わるため)
