...
async function readSourceRows(data: Buffer): Promise<SourceRow[]> {
  ...
  ws.eachRow((row, rowNumber) => {
    ...
    const [code, pref, name, prefKana, kana] = [1, 2, 3, 4, 5].map((i) =>
      cellText(row.getCell(i).value)
    )
...
async function main() {
  ...
  console.log(
    fromCache
      ...
      : `✓ fetch: 総務省から取得しキャッシュ保存 (${CACHE_PATH})`
  )
  ...
  const entries: RegistryEntry[] = base.map(({ row, slug }) => {
    ...
    const generated =
      ...
      ((count.get(slug) ?? 0) > 1
        ? `${prefectureSlugPart(row.pref, row.pref_kana)}-${slug}`
        : slug)
    return {
  ...
  const prefixed = base
    .filter((b) => (count.get(b.slug) ?? 0) > 1)
    .map((b) => entries.find((e) => e.lg_code === b.row.lg_code)!)
  console.log(`✓ emit: ${REGISTRY_PATH}`)
  console.log(
    `  件数: ${entries.length}(対応 ${entries.filter((e) => e.status === "supported").length})`
  )
  console.log(
    `  都道府県を前置した同名自治体: ${prefixed.length} 件(例: ${prefixed
      .slice(0, 6)
      .map((e) => e.slug)
      .join(", ")})`
  )
